...
# Load team configs
TEAM_CONFIGS = {}
config_paths = ['/opt/ctf/team_configs.json', './team_configs_test.json', '../team_configs_test.json']
...
class TrafficGenerator:
    ...
    def __init__(self, team_configs, mode='both', dry_run=False):
        """
    ...
    def massive_port_scan(self, fgt_ip, config):
        ...
        if self.dry_run:
            logger.info(f"[DRY RUN] Would execute nmap port scan on {fgt_ip} ports: {','.join(common_ports)}")
            return
    ...
    def apt_campaign(self, fgt_ip, config, team_id):
        ...
        if self.dry_run:
            logger.info(f"[DRY RUN] Would execute APT campaign {campaign_id} against {fgt_ip}")
            logger.info("  Stage 1: Subtle reconnaissance")
            logger.info("  Stage 2: Initial access (social engineering)")  
            logger.info("  Stage 3: Persistence establishment")
            logger.info("  Stage 4: Data collection & exfiltration")
            return
        
        logger.info(f"🎯 Starting APT campaign {campaign_id} against {fgt_ip}")
        
        # Stage 1: Reconnaissance (subtle)
        self.subtle_reconnaissance(fgt_ip, campaign_id)
        sleep_time = random.randint(5, 15) if self.dry_run else random.randint(300, 900)
        time.sleep(sleep_time)  # 5-15 min delay (shortened for dry run)
        
        # Stage 2: Initial access (social engineering simulation)
        self.initial_access(fgt_ip, campaign_id)
        sleep_time = random.randint(5, 15) if self.dry_run else random.randint(600, 1800)
        time.sleep(sleep_time)  # 10-30 min delay
        
        # Stage 3: Persistence establishment
        self.establish_persistence(fgt_ip, campaign_id)
        sleep_time = random.randint(5, 15) if self.dry_run else random.randint(900, 2700)
        time.sleep(sleep_time)  # 15-45 min delay
    ...
    def dns_tunneling(self, fgt_ip, config):
        ...
        if self.dry_run:
            logger.info(f"[DRY RUN] Would execute DNS tunneling - exfiltrating: {exfil_data}")
            logger.info(f"  Encoded data: {encoded_data[:50]}...")
            return
...
def main():
    ...
    if getattr(args, 'dry_run', False):
        logger.info("🧪 DRY RUN MODE - Simulating actions only")
        
    logger.info(f"Starting traffic generator in '{args.mode}' mode")
    gen = TrafficGenerator(TEAM_CONFIGS, mode=args.mode, dry_run=getattr(args, 'dry_run', False))
